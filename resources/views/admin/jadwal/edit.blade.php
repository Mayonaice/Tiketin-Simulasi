...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
    <h2 class="text-xl font-bold mb-4">Edit Jadwal Penerbangan</h2>
    <form action="{{ route('jadwal.update', $jadwal->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="maskapai_id" class="w-full border rounded px-3 py-2 mt-1" required>
                ...
                @foreach($maskapais as $maskapai)
                    <option value="{{ $maskapai->id }}" {{ old('maskapai_id', $jadwal->maskapai_id) == $maskapai->id ? 'selected' : '' }}>{{ $maskapai->nama_maskapai }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="kota_asal_id" class="w-full border rounded px-3 py-2 mt-1" required>
                    ...
                    @foreach($kotas as $kota)
                        <option value="{{ $kota->id }}" {{ old('kota_asal_id', $jadwal->kota_asal_id) == $kota->id ? 'selected' : '' }}>{{ $kota->nama_kota }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="kota_tujuan_id" class="w-full border rounded px-3 py-2 mt-1" required>
                    ...
                    @foreach($kotas as $kota)
                        <option value="{{ $kota->id }}" {{ old('kota_tujuan_id', $jadwal->kota_tujuan_id) == $kota->id ? 'selected' : '' }}>{{ $kota->nama_kota }}</option>
                    @endforeach
                </select>
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="tanggal_berangkat" class="w-full border rounded px-3 py-2 mt-1" required value="{{ old('tanggal_berangkat', $jadwal->tanggal_berangkat) }}">
            ...
            <div>
                ...
                <input type="time" name="jam_berangkat" class="w-full border rounded px-3 py-2 mt-1" required value="{{ old('jam_berangkat', $jadwal->jam_berangkat) }}">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="time" name="jam_tiba" class="w-full border rounded px-3 py-2 mt-1" required value="{{ old('jam_tiba', $jadwal->jam_tiba) }}">
                @error('jam_tiba')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="block text-gray-700">Harga Tiket</label>
                <input type="number" name="harga_tiket" class="w-full border rounded px-3 py-2 mt-1" required min="0" value="{{ old('harga_tiket', $jadwal->harga_tiket) }}">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="kapasitas_kursi" class="w-full border rounded px-3 py-2 mt-1" required min="1" value="{{ old('kapasitas_kursi', $jadwal->kapasitas_kursi) }}">
